...
#if USE_KHEAP
	LIST_INIT(&e->new_WS_list); //new workingset list


		struct WorkingSetElement* cop=NULL;
		//copy the working set cause i can't change the original

		LIST_FOREACH_SAFE(cop, &e->page_WS_list, WorkingSetElement)
		{
			struct WorkingSetElement*new_set=kmalloc(sizeof(struct WorkingSetElement));
			new_set->virtual_address=cop->virtual_address;
			LIST_INSERT_TAIL(&e->new_WS_list, new_set);
...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
       ...
       pt_set_page_permissions(ptr_user_page_directory,(uint32)kstack,0,PERM_PRESENT);  //should change it to zero


       return (void*)(kstack); //return including the guard page
