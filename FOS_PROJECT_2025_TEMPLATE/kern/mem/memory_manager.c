...
void initialize_paging()
{
	...
	//reverse_map = boot_allocate_space(number_of_frames * sizeof(uint32), PAGE_SIZE);
	//memset(reverse_map, 0, number_of_frames * sizeof(uint32));
}
...
int map_frame(uint32 *ptr_page_directory, struct FrameInfo *ptr_frame_info, uint32 virtual_address, int perm)
{
	...
	reverse_page_table[frame_number] = ROUNDDOWN(virtual_address, PAGE_SIZE);
...
void unmap_frame(uint32 *ptr_page_directory, uint32 virtual_address)
{
	...
	if( ptr_frame_info != 0 )
	{
		...
		/*********************************************************************************/
		reverse_page_table[frame_number] = 0;
