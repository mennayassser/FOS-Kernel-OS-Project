...
struct Share* alloc_share(int32 ownerID, char* shareName, uint32 size, uint8 isWritable)
{
	...
	struct Share* frda_shared_object = (struct Share*)kmalloc(sizeof(struct Share));
	if(frda_shared_object == NULL){
		kfree((void*)frda_shared_object); //undo if failed
		return NULL;
	}

	//references=1 ID=VA and mask most significant bit
	//awl bit zero el ba2y 1
	int32 id = ((int32)frda_shared_object) & 0x7FFFFFFF;
	frda_shared_object->ID = id;
	frda_shared_object->ownerID = ownerID;
	//shared_object->name = shareName;
	//lazem a3ml copy le string
	strcpy(frda_shared_object->name, shareName);
	frda_shared_object->size = size;
	frda_shared_object->references = 1;
	frda_shared_object->isWritable = isWritable;

	int frames_number = ROUNDUP(size,PAGE_SIZE)/PAGE_SIZE;
	//checkingg
	//check en feeh free frames tkafy
	struct freeFramesCounters counter = calculate_available_frames();
	if(frames_number> (counter.freeBuffered + counter.freeNotBuffered)){
		return NULL;
	}
	int frames_size = frames_number* sizeof(struct FrameInfo*);
	frda_shared_object->framesStorage =(struct FrameInfo**) kmalloc(frames_size);
	if(frda_shared_object->framesStorage == NULL){
			kfree((void*)frda_shared_object); //undo if failed
			return NULL;
	}
	for(int i=0;i<frames_number;i++){
		frda_shared_object->framesStorage[i] = 0;
	}


	return frda_shared_object;
...
int create_shared_object(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	//checkingg
	//mynfa3sh a3ml create le 7aga mwgooda already
	struct Share* share = find_share(ownerID, shareName);
	if (share != NULL)
		return E_SHARED_MEM_EXISTS;

	struct Share* frda_shared_object = alloc_share(ownerID,shareName,size,isWritable);
	if(frda_shared_object==NULL){
		return E_NO_SHARE;
	}

	acquire_kspinlock(&(AllShares.shareslock));
	LIST_INSERT_TAIL(&(AllShares.shares_list), frda_shared_object);
	release_kspinlock(&(AllShares.shareslock));

	int frames_number = ROUNDUP(size,PAGE_SIZE)/PAGE_SIZE;
	for(int i=0;i<frames_number;i++){
		allocate_frame(&(frda_shared_object->framesStorage[i]));
	}

	//el process ely bt3ml create lazem tkon writable
	uint32 frda_perm = PERM_PRESENT|PERM_USER|PERM_UHPAGE|PERM_WRITEABLE;
	for(int i=0;i<frames_number;i++){
	uint32 va = (uint32)virtual_address+ (i*PAGE_SIZE);
	map_frame(myenv->env_page_directory,frda_shared_object->framesStorage[i],va, frda_perm);
	}
...
int get_shared_object(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	//checkingg
	struct Share* frda_shared_object = find_share(ownerID,shareName);
	if(frda_shared_object==NULL){
		return E_SHARED_MEM_NOT_EXISTS;
	}

	struct FrameInfo**frames = frda_shared_object->framesStorage;

	//lazem a-check htb2a writable wala read only(isWritable)
	uint32 frda_perm = PERM_PRESENT|PERM_USER|PERM_UHPAGE;
	if(frda_shared_object->isWritable){
		frda_perm |= PERM_WRITEABLE;
	}

	int frames_number = ROUNDUP(frda_shared_object->size,PAGE_SIZE)/PAGE_SIZE;
	for(int i=0;i<frames_number;i++){
		uint32 va=(uint32)virtual_address +(i*PAGE_SIZE);
		map_frame(myenv->env_page_directory,frames[i],va, frda_perm);
	}

	frda_shared_object->references++;
