...
void env_set_priority(int envID, int priority)
{
    ...
    acquire_kspinlock(&ProcessQueues.qlock);
        ...
        p->env_id = envID;
    	p->priority = priority;
              //delw'ty mehtaga ashuf eh ready ashelo then a update priority w arg3o fe mkano al gded
        		if (p->env_status == ENV_READY) {
        			sched_remove_ready(p);
        			p->priority = priority;
